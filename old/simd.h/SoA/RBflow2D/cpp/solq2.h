...
void solq2k()  /* checked */
{
  ...
  static vreal cnst[NY] __attribute__((aligned(64))) ;
...
#pragma omp parallel for
  ...
  {
    const vreal ackl_b = vreal(1.0) - VLDA(ac3sk(kc))*betadx;
    const vreal inv    = vreal(1.0)/ackl_b;
    const vreal a      = -VLDA(am3sk(kc)) * betadx * inv;
    const vreal c      = -VLDA(ap3sk(kc)) * betadx * inv;
