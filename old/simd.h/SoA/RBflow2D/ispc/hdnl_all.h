...
void hdnl_all_scal(const int kbeg, const int kend, const int jbeg, const int jend)
{
  ...
  for (int kc = kbeg; kc < kend; kc++)
  {
    /*    h term for the q2 momentum equation at i+1/2,j,k+1/2     */
    {
      ...
      for (int jc = jbeg; jc < jend; jc++)
      {
         ...
         *                 d   r      
         */
        const real h22 = (-0.25)*udx2c(jc)*
          (SQR(q2(jp,kc)+q2(jc,kc)) - SQR(q2(jc,kc)+q2(jm,kc)));
...
void hdnl_all()
{
  ...
  hdnl_all_scal(0, n3m, 0,  SIMD::WIDTH);
  hdnl_all_scal(0, n3m, n2m-SIMD::WIDTH, n2m);
